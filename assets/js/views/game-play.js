...
export async function getView(ctx, age, skill, slug) {
  ...
  if (window.UEAH_GAMES_STORE) {
    // For featured games, use getGameBySlug instead
    if (age === 'featured') {
      game = window.UEAH_GAMES_STORE.getGameBySlug(slug);
    } else {
  ...
  // Build breadcrumbs - simplified for featured games
  const breadcrumb = age === 'featured' ? breadcrumbs([
    { label: 'Home', href: hrefFor('/') },
    { label: 'Games', href: hrefFor('/games') },
    { label: game.title },
  ]) : breadcrumbs([
